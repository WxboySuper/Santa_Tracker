...
- **Feature flags (governance):** Four scaffold flags (`adventEnabled` default false, `mapEnabled` true, `weatherEnabled` false, `soundscapeEnabled` false) registered in `FEATURE_FLAG_REGISTRY` with owner `foundation`, exposure `publication`, validated at publication; not yet visitor-facing. Disclosed in `README.md` flags table and this changelog; changes require ADR.
...
- **Rollout/Migration:** No DB migration. Legacy `src/`, `tests/`, `tools/route-editor` remain (legacy JS still linted via `disableTypeChecked`). Risk low; rollback via `git revert`. Build is `standalone` on Linux (VPS); Windows local build skips standalone to avoid EPERM.
