...
To add a message, add a typed key to `packages/localization/src/messages.ts`, use `t("key")` at the UI boundary, and add a fallback test when the message has interpolation or special locale behavior. English is the only shipped catalog in this PR. Additional catalogs can be supplied to `createTranslator` without changing component code.
