...
export const useMonitorReferenceLayers = ({
  ...
  useEffect(() => {
    ...
    if (!ndfdEnabled) {
      setState((current) => ({ ...current, ndfdTemperature: createMeta({
        status: 'idle',
        sourceName: NDFD_TEMPERATURE_SOURCE.sourceName,
        sourceUrl: NDFD_TEMPERATURE_SOURCE.sourceUrl,
        attribution: NDFD_TEMPERATURE_SOURCE.attribution,
  ...
  useEffect(() => {
    ...
    const now = Date.now();
    const cacheIsFresh = cachedSpcSnapshot && now - cachedSpcSnapshot.fetchedAt <= REFERENCE_CACHE_TTL_MS && refreshToken === 0;
    if (cacheIsFresh) {
      ...
      setState((current) => ({
        ...
        spcMesoscaleDiscussion: createMeta({
          status: cached.collection.features.length > 0 ? 'ready' : 'empty',
          sourceName: SPC_MESOSCALE_DISCUSSION_SOURCE.sourceName,
          sourceUrl: SPC_MESOSCALE_DISCUSSION_SOURCE.sourceUrl,
          attribution: SPC_MESOSCALE_DISCUSSION_SOURCE.attribution,
    ...
    setState((current) => ({ ...current, spcMesoscaleDiscussion: createMeta({
      ...current.spcMesoscaleDiscussion,
      status: 'loading',
      error: null,
    }) }));
    ...
    fetchSpcMesoscaleDiscussions()
      .then((collection) => {
        ...
        setState((current) => ({
          ...
          spcMesoscaleDiscussion: createMeta({
            status: collection.features.length > 0 ? 'ready' : 'empty',
            sourceName: SPC_MESOSCALE_DISCUSSION_SOURCE.sourceName,
            sourceUrl: SPC_MESOSCALE_DISCUSSION_SOURCE.sourceUrl,
            attribution: SPC_MESOSCALE_DISCUSSION_SOURCE.attribution,
      ...
      .catch((error: unknown) => {
        ...
        if (cached && cacheAge <= REFERENCE_STALE_WINDOW_MS) {
          setState((current) => ({
            ...current,
            mesoscaleDiscussions: cached.collection,
            spcMesoscaleDiscussion: createMeta({
          ...
          setState((current) => ({ ...current, mesoscaleDiscussions: emptyCollection(), spcMesoscaleDiscussion: createMeta({
            status: 'error',
            sourceName: SPC_MESOSCALE_DISCUSSION_SOURCE.sourceName,
            sourceUrl: SPC_MESOSCALE_DISCUSSION_SOURCE.sourceUrl,
            attribution: SPC_MESOSCALE_DISCUSSION_SOURCE.attribution,
