...
import { normalizeProductionReleaseConfig, validateProductionReleaseForDeploy } from '../server/lib/production-release.mjs';
...
const readJsonAtRef = (ref, path) => JSON.parse(execFileSync('git', ['show', `origin/${ref}:${path}`], { encoding: 'utf8' }));
...
if (current[1] !== previous[1] || current[2] !== previous[2] || Number(current[3]) <= Number(previous[3])) {
  console.error(`Stable hotfix PRs must advance the patch version from ${basePackage.version} to a newer ${current[1]}.${current[2]}.Y version.`);
  process.exit(1);
}
...
const manifest = normalizeProductionReleaseConfig(JSON.parse(readFileSync('deploy/production-release.json', 'utf8')));
