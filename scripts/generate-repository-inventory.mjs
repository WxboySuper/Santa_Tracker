...
async function resolveImport(importerPath, specifier, knownFiles) {
  ...
  for (const extension of CODE_EXTENSIONS) {
    const repositoryPath = toRepositoryPath(path.join(base, `index${extension}`));
    if (knownFiles.has(repositoryPath)) return repositoryPath;
  }
  return null;
}
...
function mermaidFor(inventory) {
  const id = (name) => name.replaceAll('/', '_').replaceAll('-', '_');
  return ['%% Generated by pnpm run docs:inventory. Do not edit.', 'flowchart LR', ...inventory.boundaries.map(({ name }) => `  ${id(name)}["${name}"]`), ...inventory.dependencyEdges.map(({ from, to }) => `  ${id(from)} --> ${id(to)}`), ''].join('\n');
}
