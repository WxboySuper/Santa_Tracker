...
function inlineMarkdown(value) {
  ...
  html = html.replace(/`([^`]+)`/g, '<code>$1</code>').replace(/!\[([^\]]*)\]\(([^)]+)\)/g, '<img alt="$1" src="$2">').replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2">$1</a>').replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>').replace(/\*([^*]+)\*/g, '<em>$1</em>');
  return html;
}
...
function renderTable(lines) {
  ...
  const renderCells = (row, tag) => row.map((cell) => `<${tag}>${inlineMarkdown(cell)}</${tag}>`).join('');
...
function createState() { return { html: [], paragraph: [], list: null, fence: null, fenceLines: [] }; }
...
function consumeHeading(state, line) {
  ...
  if (!match) return false;
  flushParagraph(state); flushList(state); const level = match[1].length; state.html.push(`<h${level}>${inlineMarkdown(match[2])}</h${level}>`); return true;
}
...
function pageHtml(title, relativePath, body) { return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)} · GFC docs</title><link rel="stylesheet" href="site.css"></head><body><header><a href="index.html">GFC local docs</a><span>${escapeHtml(relativePath)}</span></header><main><article>${body}</article></main><script type="module">import mermaid from 'https://cdn.jsdelivr.net/npm/mermaid@11/dist/mermaid.esm.min.mjs'; mermaid.initialize({startOnLoad:true,securityLevel:'strict'});</script></body></html>`; }

/** Build the ignored local documentation site. */
async function main() {
  const files = await sourceFiles(); await fs.rm(OUTPUT, { recursive: true, force: true }); await fs.mkdir(OUTPUT, { recursive: true }); const pages = [];
  for (const file of files) { const title = file.relative.replace(/\.md$/i, '').split('/').pop(); const slug = pageSlug(file.relative); await fs.writeFile(path.join(OUTPUT, slug), pageHtml(title, file.relative, renderMarkdown(await fs.readFile(file.absolute, 'utf8')))); pages.push({ title, path: file.relative, href: slug }); }
